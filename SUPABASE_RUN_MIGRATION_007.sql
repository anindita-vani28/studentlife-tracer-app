...
-- This creates all tables for internships, opportunities, discussions, and movies
...
CREATE INDEX IF NOT EXISTS idx_internship_follow_ups_user_id ON internship_follow_ups(user_id);

ALTER TABLE internship_applications ENABLE ROW LEVEL SECURITY;
ALTER TABLE internship_follow_ups ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own internship applications" ON internship_applications;
DROP POLICY IF EXISTS "Users can insert their own internship applications" ON internship_applications;
DROP POLICY IF EXISTS "Users can update their own internship applications" ON internship_applications;
DROP POLICY IF EXISTS "Users can delete their own internship applications" ON internship_applications;

CREATE POLICY "Users can view their own internship applications"
  ON internship_applications FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own internship applications"
  ON internship_applications FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own internship applications"
  ON internship_applications FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own internship applications"
  ON internship_applications FOR DELETE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view their own internship follow-ups" ON internship_follow_ups;
DROP POLICY IF EXISTS "Users can insert their own internship follow-ups" ON internship_follow_ups;
DROP POLICY IF EXISTS "Users can update their own internship follow-ups" ON internship_follow_ups;
DROP POLICY IF EXISTS "Users can delete their own internship follow-ups" ON internship_follow_ups;

CREATE POLICY "Users can view their own internship follow-ups"
  ON internship_follow_ups FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own internship follow-ups"
  ON internship_follow_ups FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own internship follow-ups"
  ON internship_follow_ups FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own internship follow-ups"
  ON internship_follow_ups FOR DELETE USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "Users can view their own saved opportunities" ON saved_opportunities;
...
INSERT INTO opportunities (title, organizer, category, deadline, event_type, location, description, official_url) VALUES
  ...
  ('Jane Street Internship', 'Jane Street', 'internship', '2026-09-30', 'online', 'New York, NY', 'Summer internship program for students', 'https://janestreet.com')
...
CREATE INDEX IF NOT EXISTS idx_discussion_posts_created ON discussion_posts(created_at);
CREATE INDEX IF NOT EXISTS idx_discussion_comments_post_id ON discussion_comments(post_id);
...
DROP POLICY IF EXISTS "Anyone can view posts" ON discussion_posts;
DROP POLICY IF EXISTS "Users can create posts" ON discussion_posts;
...
CREATE POLICY "Users can update own posts"
  ON discussion_posts FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own posts"
  ON discussion_posts FOR DELETE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Anyone can view comments" ON discussion_comments;
...
CREATE POLICY "Users can create comments"
  ON discussion_comments FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own comments"
  ON discussion_comments FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own comments"
  ON discussion_comments FOR DELETE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Anyone can view reactions" ON discussion_reactions;
DROP POLICY IF EXISTS "Users can create reactions" ON discussion_reactions;
DROP POLICY IF EXISTS "Users can delete own reactions" ON discussion_reactions;

CREATE POLICY "Anyone can view reactions"
  ON discussion_reactions FOR SELECT USING (TRUE);

CREATE POLICY "Users can create reactions"
  ON discussion_reactions FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own reactions"
  ON discussion_reactions FOR DELETE USING (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS idx_movie_watchlist_status ON movie_watchlist(status);

ALTER TABLE movie_watchlist ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own watchlist" ON movie_watchlist;
DROP POLICY IF EXISTS "Users can add to watchlist" ON movie_watchlist;
DROP POLICY IF EXISTS "Users can update their watchlist" ON movie_watchlist;
DROP POLICY IF EXISTS "Users can remove from watchlist" ON movie_watchlist;

CREATE POLICY "Users can view their own watchlist"
  ON movie_watchlist FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can add to watchlist"
  ON movie_watchlist FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their watchlist"
  ON movie_watchlist FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can remove from watchlist"
  ON movie_watchlist FOR DELETE USING (auth.uid() = user_id);
