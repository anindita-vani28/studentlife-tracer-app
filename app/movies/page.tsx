...
const CATEGORIES = ['Technology', 'Science', 'Business', 'Entrepreneurship', 'History', 'Motivation', 'Psychology', 'Creativity', 'Inspiration', 'Sci-Fi', 'Documentaries', 'Education', 'Mathematics', 'Mental Health']
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-8">
          ...
          <button onClick={() => setView('watchlist')} className={`px-6 py-2 rounded-lg font-semibold ${view === 'watchlist' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border'}`}>My Watchlist</button>
        </div>
                  ...
                  <div key={movie.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition">
                    {movie.poster_url && (
                      <div className="relative h-48 bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center">
                        <div className="text-gray-600">📽️ {movie.title}</div>
                      </div>
                    )}
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-lg font-bold text-gray-900">{movie.title}</h3>
                        <button onClick={() => toggleWatchlist(movie.id, watchlist.has(movie.id))} className={`text-2xl ${watchlist.has(movie.id) ? 'text-red-500' : 'text-gray-300'}`}>❤️</button>
                      </div>
        ...
        {view === 'watchlist' && (
          <div>
            {watchlist.size === 0 ? (
              <div className="bg-white rounded-lg shadow p-8 text-center"><h3 className="text-lg font-semibold text-gray-900">Your watchlist is empty</h3><p className="text-gray-600">Start adding movies to your watchlist!</p></div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {movies.filter(m => watchlist.has(m.id)).map((movie) => (
                  <div key={movie.id} className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-lg font-bold text-gray-900">{movie.title}</h3>
                        <button onClick={() => toggleWatchlist(movie.id, true)} className="text-2xl text-red-500">❤️</button>
                      </div>
