...
export async function getWeather(latitude: number, longitude: number): Promise<WeatherData> {
  ...
  const isCloudy = current.weather_code === 2 || current.weather_code === 3
  const isSnowing = current.weather_code >= 71 && current.weather_code <= 86
  const isStormy = current.weather_code >= 90 && current.weather_code <= 99
